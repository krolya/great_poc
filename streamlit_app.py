...
def GeneratePerson():
    ...
    prompt = f"""
    Ты специальный сервис по созданию персонажей и генерацией их ответов на несколько вопросов по поводу рекламы. Твоя задача сгенерировать JSON-объект
    с случайными персонажами по следующим правилам:
    ...
    После генерации персонажей, ты должен представить себя на месте сгенерированного персонажа и внимательно прочитать описание рекламы, которое дано ниже:
     {ad_description} 
     
    После этого задай им следующие вопросы (если вопроса нет, то просто пропускаем его):
    ...
    {{
        ...
        {{
            ...
            "Marital status": "В браке", #случайное семейное положение из списка {marital_selected}
            ...
            "Description": "Здесь нужно дать полное описание персоны с учетом всех параметров выше", #полное описание персонажа
